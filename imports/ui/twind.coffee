...
options =
  ...
  preflight: (preflight, {theme}) -> {
    ...
    'input': apply"border border-gray-300 rounded bg-white text-gray-800 w-full #{css"padding: .25rem 1rem !important"}"
    'input[type="submit"]': apply"rounded bg-blue-500 text-white w-auto"
    button: apply"rounded bg-gray-300 #{css"padding: .25rem 1rem"}"
    '.u-nested': apply"border border-gray-100 rounded p-2 m-"
